...
void insertAtIndex(ListNode*& head, int val, int i) {
    ...
    // 3. update the next field of the newly created node such that it now points to the node which comes after the prev node
    n->next = prev->next;

    // 4. update the next field of the prev node such that it now points to the newly created node
    prev->next = n;
}
